define([
  ...
  /**
   * Used for Storing Config Parameters fetched from meta tags
   * @type {json}
   */
  self.config = {};

  /**
   * Used for Storing API IDs fetched from a meta tag
   * @type {json}
   */
  self.apiIds = {};

  /**
   * Used for Storing AB tests fetched from a meta tag
   * @type {json}
   */
  self.abTests = {};
  ...
  var initApi = function(ready, apiId, scriptUrls, logger, initCallback) {
    ...
    if (!self.config.enabled) {
      logger.debug.log("Analytics is globally disabled");
      ready.reject();
      return ready;
    }

    if (!apiId) {
      logger.debug.log("Failed to initialize because an API ID was not provided");
      ready.reject();
      return ready;
    }
    ...
    $.when.apply($, _.map(scriptUrls, function(url) { return $.getScript(url); }))
      ...
      logger.debug.log("Failed to Load Script - Check Ad Blocker");
      ready.reject();
    });
  ...
  return {
    ...
    getApiId: function (key) {
      return self.apiIds[key];
    },
    getAbTests: function () {
      return self.abTests;
    },
